...
import { Account, Metadata, User } from "@/schema/entity";
import { Badge, Button, 
        Select, 
        Table, 
        TableBody, 
...
const AccountList = ({ accounts, metadata } : AccountListProps):JSX.Element => {
    ...
    const runFilter = () => {
        ...
        const filterEndpoint = `/account?${params.toString()}`;
                ...
                <TableBody className="divide-y">
                    ...
                    {accounts.map((account: Account, index: number) => (
                        <TableRow key={index}>
                                ...
                                <TableCell className={account.isActive ? `bg-green-100 text-green-800 font-medium`: `bg-gray-100 text-gray-800 font-medium`}>
                                    {account.isActive ? 'Active' : 'Inactive'}
                                </TableCell>
                                <TableCell><Button color={"light"} as={Link} href={`/account/${account.id}`} size="sm"><HiPencilAlt className="mr-3" /> View Data</Button></TableCell>
                        </TableRow>
                    ))}
                </TableBody>
            </Table>
